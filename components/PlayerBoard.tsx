        ...
        {players.sort((a, b) => b.score - a.score).map((p) => (
          ...
          >
            <div className="relative">
              ...
              {!p.isConnected && (
                <div className="absolute -bottom-1 -right-1 bg-slate-500 rounded-full w-3 h-3 border border-slate-900" />
              )}
            </div>
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-1">
                <p className="font-medium truncate text-slate-200">
                  ...
                  {!p.isConnected && <span className="text-[10px] text-slate-400 font-bold ml-1 uppercase">Away</span>}
                </p>
                {p.isGuesser && <Eye size={14} className="text-purple-400" />}
              </div>
              <p className="text-xs text-slate-400 font-semibold">{p.score} Stars</p>
            </div>
            {p.isConnected && p.hasFinishedDrawing && (
